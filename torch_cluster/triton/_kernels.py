...
def _nearest_kernel(
    ...
    """
    pid = tl.program_id(0)  # Program id over x blocks.
    offs_n = pid * BLOCK_N + tl.arange(0, BLOCK_N)  # X indices for this block.
    mask_x = offs_n < N  # Mask for valid x rows.
    if EVEN_N:
        tl.multiple_of(offs_n, 8)  # Hint contiguous access.
        tl.max_contiguous(offs_n, 8)  # Hint vectorization.

    best_dist = tl.full((BLOCK_N,), float('inf'), tl.float32)  # Best distance per x.
    best_idx = tl.zeros((BLOCK_N,), dtype=tl.int32)  # Best y index per x.
    ...
    if USE_BATCH:
        ...
        left = 0  # Full y range when not batched.
        right = M

    x_sq = tl.zeros((BLOCK_N,), dtype=tl.float32)  # Precompute ||x||^2 once.
    ...
    for _ in range(0, D, BLOCK_K):
        if EVEN_N and EVEN_K:
            x = tl.load(x_block_ptr_sq)  # Full tile load for x.
        else:
            x = tl.load(x_block_ptr_sq,  # Tail handling in N or D.
                        boundary_check=(0, 1),
                        padding_option="zero")
        x_sq += tl.sum(x * x, axis=0)  # Accumulate ||x||^2.
        x_block_ptr_sq = tl.advance(x_block_ptr_sq, (BLOCK_K, 0))  # Next K tile.
    if COSINE:
        inv_x = tl.rsqrt(x_sq + EPS)  # Precompute 1/||x||.

    x_block_ptr_base = tl.make_block_ptr(  # Base pointer for dot loop.
            base=x_ptr,
            shape=(D, N),
            strides=(stride_xd, stride_xm),
            offsets=(0, pid * BLOCK_N),
    ...
    for y_start in range(0, M, BLOCK_M):
        offs_m = y_start + tl.arange(0, BLOCK_M)  # y indices for this tile.
        mask_y = offs_m < M  # Mask for valid y rows.
        full_y = y_start + BLOCK_M <= M  # Full tile fast path.

        acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)  # Dot accumulator.
        ...
        for _ in range(0, D, BLOCK_K):
            if full_y and EVEN_K:
                y = tl.load(y_block_ptr)  # Full tile load for y.
                if EVEN_N:
                    x = tl.load(x_block_ptr)  # Full tile load for x.
                else:
                    x = tl.load(x_block_ptr,  # Tail in N only.
                                boundary_check=(0, 1),
                                padding_option="zero")
            else:
                y = tl.load(y_block_ptr,  # Tail in M or D.
                            boundary_check=(0, 1),
                            padding_option="zero")
                x = tl.load(x_block_ptr,  # Tail in N or D.
                            boundary_check=(0, 1),
                            padding_option="zero")
            acc += tl.dot(y, x, input_precision="ieee")  # Accumulate dot.
            y_sq += tl.sum(y * y, axis=1)  # Accumulate ||y||^2.
            y_block_ptr = tl.advance(y_block_ptr, (0, BLOCK_K))  # Next K tile.
            x_block_ptr = tl.advance(x_block_ptr, (BLOCK_K, 0))

        if COSINE:
            inv_y = tl.rsqrt(y_sq + EPS)  # 1/||y||.
            dist = 1.0 - acc * (inv_y[:, None] * inv_x[None, :])  # Cosine distance.
        else:
            dist = tl.fma(-2.0, acc, y_sq[:, None] + x_sq[None, :])  # L2^2 distance.

        if full_y:
            valid = tl.broadcast_to(mask_x[None, :], (BLOCK_M, BLOCK_N))  # No y mask.
        else:
            valid = mask_y[:, None] & mask_x[None, :]  # Tail y mask.
        if USE_BATCH:
            valid &= (offs_m[:, None] >= left[None, :]) & (offs_m[:, None] < right[None, :])
        dist = tl.where(valid, dist, float('inf'))  # Mask invalid distances.

        block_min, block_arg = tl.min(  # Best y in this tile.
            dist, axis=0, return_indices=True,
            return_indices_tie_break_left=True)
        block_idx = y_start + block_arg  # Convert to global y index.
        better = block_min < best_dist  # Compare to current best.
...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_N': 32, 'BLOCK_D': 16}, num_warps=2, num_stages=2),
        triton.Config({'BLOCK_N': 32, 'BLOCK_D': 16}, num_warps=2, num_stages=3),
        triton.Config({'BLOCK_N': 32, 'BLOCK_D': 32}, num_warps=2, num_stages=2),
        triton.Config({'BLOCK_N': 64, 'BLOCK_D': 16}, num_warps=4, num_stages=2),
        triton.Config({'BLOCK_N': 64, 'BLOCK_D': 32}, num_warps=4, num_stages=3),
        triton.Config({'BLOCK_N': 64, 'BLOCK_D': 64}, num_warps=4, num_stages=3),
        triton.Config({'BLOCK_N': 128, 'BLOCK_D': 32}, num_warps=8, num_stages=3),
        triton.Config({'BLOCK_N': 128, 'BLOCK_D': 64}, num_warps=8, num_stages=4),
        triton.Config({'BLOCK_N': 256, 'BLOCK_D': 32}, num_warps=8, num_stages=4),
    ],
    key=['D', 'MAX_CAND'],
...
@triton.heuristics({
    ...
    'MAX_X_BLOCKS': lambda args: triton.cdiv(args['MAX_CAND'], args['BLOCK_N']),
...
def _knn_segmented_kernel(
    ...
    example_idx = tl.load(example_idx_ptr + n_y, mask=mask_y, other=0)  # Segment id.
    x_start = tl.load(ptr_x_ptr + example_idx, mask=mask_y, other=0)  # Segment start.
    x_end = tl.load(ptr_x_ptr + example_idx + 1, mask=mask_y, other=0)  # Segment end.
    ...
    for yi in tl.static_range(NUM_D_BLOCKS):
        if EVEN_D:
            ...
            y = tl.load(y_block_ptr, boundary_check=(0, 1), padding_option="zero")  # Tail D.
    ...
    for xb in range(MAX_X_BLOCKS):
        ...
        full_block = (x_block_start + BLOCK_N <= x_end) & (x_block_start + BLOCK_N <= N) & mask_y
        mask_x = tl.where(full_block, mask_y, (x_idx < x_end) & (x_idx < N) & mask_y)
        if MAX_X_BLOCKS > 1:
        ...
        x_block_start_i32 = x_block_start.to(tl.int32)  # Block ptr needs int32 offsets.
        ...
        for nd in tl.static_range(NUM_D_BLOCKS):
            if EVEN_D:
                ...
                x = tl.load(x_block_ptr, boundary_check=(0, 1), padding_option="zero")  # Tail D.
        ...
        dist_k = (key_k >> 32).to(tl.int32).to(tl.float32, bitcast=True)  # Unpack dist.
        idx_k = (key_k & 0xFFFFFFFF).to(tl.int32)  # Unpack idx.
        block_top_dist = tl.where(k_valid, dist_k, float("inf"))  # Block top-k dist.
        ...
        combo_dist = tl.where(k_offsets < K, best_dist, block_shifted_dist)  # Merge buffers.
        combo_idx = tl.where(k_offsets < K, best_idx, block_shifted_idx)
        dist_bits = combo_dist.to(tl.int32, bitcast=True).to(tl.int64)  # Pack combo.
    ...
    best_idx = tl.where(libdevice.isinf(best_dist), -1, best_idx)  # Mask invalid.
    ...
    tl.store(col_ptr + out_offsets, best_idx.to(tl.int64), mask=out_mask)  # Write col idx.
